...
class BaseDevModule {
    ...
    attachPoint() {
        ...
        this.compiler.plugin(
            "emit",
            this.wrapGen(function*(compilation, cb) {
                //清除掉assets.js相关文件
                try {
                    ...
                    _.isFunction(this.pluginOption.onEmitAssets) &&
                        this.pluginOption.onEmitAssets.call(
                            this,
                            compilation.assets
                        );
                    ...
                    console.error(
                        chalk.red(
                            `[Error] emit assets Error: ${e},trace: ${e.stack}`
                        )
                    );
                }
            })
    ...
    *emitAssets(compilation) {}
    ...
    addAssetsEntry(assets = []) {
        ...
        // });
        this.compiler.apply(
            new MultiEntryPlugin(this.getProjectRoot(), extraAssets, assetsName)
        );
    }
                ...
                return function*() {
                    try {
                        assets[name] = new RawSource(
                            yield fs.readFile(
                                path.join(this.getProjectRoot(), name),
                                "utf-8"
                            )
                        );
                    } catch (e) {}
            ...
            : (module, count) => {
                  if (module.resource) {
                      ...
                      if (ext) {
                          ext = ext.replace(/^\./, "");
                          if (
                              (<any>this.pluginOption).originExt.indexOf(ext) >
                              -1
                          ) {
                              return (
        ...
        compilation.mainTemplate.plugin("bootstrap", (core, { name }) => {
            ...
            if (name === this.getCommonName()) {
                ...
                acorn.parse(source, {
                    onToken: token => {
                        if (
                            ["window", "webpackJsonp"].indexOf(token.value) > -1
                        ) {
                            arr.push(token);
                        }
                    }
