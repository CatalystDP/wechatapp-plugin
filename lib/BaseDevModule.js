...
class BaseDevModule {
    ...
    *appendAsset(compilation) {
    ...
    appendCommonPlugin(name) {
        ...
        let defaultCommonsChunkConfig = {
            name,
            minChunks: this.pluginOption.hasOwnProperty('minChunks') ? this.pluginOption.minChunks : (module, count) => {
        ...
        if (_.isObject(this.pluginOption.defaultCommonsChunkConfig)) {
            defaultCommonsChunkConfig = _.extend(defaultCommonsChunkConfig, this.pluginOption.defaultCommonsChunkConfig);
        }
        if (Array.isArray(this.pluginOption.extraCommonsChunkPluginsConfig) && this.pluginOption.injectEntry) {
            //extraCommonsChunkPluginConfig 必须要配合injectEntry来使用
            this.pluginOption.extraCommonsChunkPluginsConfig.forEach(pl => {
                if (_.isString(pl.name) && !_.isEmpty(pl.name)) {
                    this.compiler.apply(new CommonsChunkPlugin(pl));
                }
            });
        }//额外的plugin
        this.compiler.apply(new CommonsChunkPlugin(defaultCommonsChunkConfig));//
    }
    ...
    getCommonAbsolute(commonName) {
        return path.join(this.distPath, `${commonName}${this.pluginOption.ext}`);
    }
    getCommonName() {
        return '';
    }
    ...
    attachTemplatePlugin(compilation, params) {
        ...
        compilation.chunkTemplate.plugin('render', (core, { name }) => {
            let localCompilation = compilation;
            let source = core;
            let injectContent = [
            ...
            /**@type {array} */
            let injectEntry;
            if (_.isPlainObject(this.pluginOption.injectEntry)) {
                injectEntry = [this.pluginOption.injectEntry];
            } else if (_.isArray(this.pluginOption.injectEntry)) {
                injectEntry = this.pluginOption.injectEntry;
            }
            if (Array.isArray(injectEntry)) {
                injectEntry.forEach(entry => {
                    if (!_.isObject(entry)) return;
                    let { name: entryName, chunks } = entry;
                    if (!entryName || !Array.isArray(chunks)) return;
                    if (name != entryName) return;
                    chunks.filter(chunk => {
                        return localCompilation.chunks.some(c=>{
                            return c.name == chunk;//必须在compilation 里面chunk才能注入到当前指定的入口
                        });
                    }).forEach(chunk => {
                        let p = this.getCommonRelativePath(chunk, path.join(this.distPath, `${entryName}${this.pluginOption.ext}`));
                        injectContent.push(`require(${JSON.stringify(p)});`);
                    });
                });
            }
            let injectFunction = `
